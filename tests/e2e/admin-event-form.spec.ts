...
test.describe('Event Form E2E', () => {
  ...
  test('validation errors display for invalid data', async ({ page }) => {
    ...
    await expect(page.locator('text=required')).toBeVisible({ timeout: 3000 }).catch(() => {
  ...
  test('slug preview updates with event name', async ({ page }) => {
    ...
    const slugInput = page.locator('input[readonly][value*="events/"]').or(
      page.locator('div:has-text("URL Slug") input')
    );
